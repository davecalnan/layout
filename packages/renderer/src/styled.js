...
const extractBooleanPropTypes = Component =>
  Object.entries(Component.propTypes || {}).filter(
    ([propName, propType]) => propType.type === 'boolean'
  )

const extractEnumPropTypes = Component =>
  Object.entries(Component.propTypes || {}).filter(
...
export const styled = Component => {
  return (...inputCSS) => {
    const annoyingObjectSoThatICanNameTheFunctionDynamically = {
      [Component.name]: props => {
        const variantClassNames = [
          ...extractBooleanPropTypes(Component),
          ...extractEnumPropTypes(Component)
    ...
    const Output =
      annoyingObjectSoThatICanNameTheFunctionDynamically[Component.name]
    ...
    root.walkRules(rule => {
      const getBaseSelector = selector => {
        const variantSelector =
          (selector.match(/(.[^ ]*)( )/, (match, target) => target) || [])[1] ||
          selector
        const [_, baseSelector] =
          selector.match(/(.[a-zA-Z]*)(__)/, (match, target) => target) || []
